...
import {answerAdd, viewTask} from "./utils/_data";
...
class Answers extends React.Component {
   ...
   /* compare(a,b) {
    ...
    onFormSubmit = async (event) =>{
        ...
        console.log(this.uploadInput.files[0],"Image");
        ...
        if(response?.data?.status_code===200 || response?.data?.status_code===201 ) {
            that.setState({uploadedSuccessfully:true, isLoading: false, message:response?.data?.message})
        }else if(response?.data?.status_code===500) {
            that.setState({uploadedUnsuccessful:true, isLoading: false, message:"Please select a file to upload."})
        } else {
            that.setState({uploadedUnsuccessful:true, isLoading: false, message:response.data.message[0].msg})
        }
        ...
        //         that.setState({uploadedSuccessfully:true, isLoading: false, message:data.message})
        //       }else if(data.status_code==500) {
        //         that.setState({uploadedUnsuccessful:true, isLoading: false, message:"Please select a file to upload."})
        //       } else {
        //         that.setState({uploadedUnsuccessful:true, isLoading: false, message:data.message[0].msg})
        //       }
        //     }).catch((err) => {
        //         console.log(err);
      ...
      renderSubmissionView(obj,index) {
        ...
        var menteeProfilePic;
          ...
          if(obj.submittedBy === 'mentor') {
            ...
            }else if(this.state.data.isAnonymous === true && obj.submittedBy === 'user'){  //anonymous check
                menteeName = 'Mentee(Anonymous)';
                menteeProfilePic = userDefaultImage
            }else if(this.state.data.isAnonymous === false && obj.submittedBy === 'user'){
